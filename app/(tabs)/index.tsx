...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, TextInput, Alert, StatusBar, Vibration, RefreshControl } from 'react-native';
...
const PantryItemCard = ({ item, onEdit, onDelete, theme }: { item: PantryItem, onEdit: (item: PantryItem) => void, onDelete: (id: number) => void, theme: any }) => {
...
const LocationFilter = ({ locations, selectedLocation, onSelect, theme }: { locations: string[]; selectedLocation: string; onSelect: (location: string) => void; theme: any }) => {
  return (
    <FlatList 
      horizontal 
...
export default function PantryScreen() {
  ...
  React.useEffect(() => {
    ...
    const startPulseAnimation = () => {
      pulseAnim.value = withSequence(
        ...
        withDelay(500, withTiming(1, { duration: 0 }, (finished) => {
          if (finished) {
            startPulseAnimation();
          }
        }))
  ...
  const statusCounts = {
    fresh: items.filter(item => getExpirationStatus(item.expiryDate, theme).status === 'Fresh').length,
    expiring: items.filter(item => getExpirationStatus(item.expiryDate, theme).status === 'Expiring Soon').length,
    expired: items.filter(item => getExpirationStatus(item.expiryDate, theme).status === 'Expired').length,
  };
      ...
      <View style={styles.overviewContainer}>
        <View style={styles.overviewCards}>
          <View style={[styles.overviewCard, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.overviewNumber, { color: theme.colors.success }]}>
              {statusCounts.fresh}
            </Text>
          ...
          <View style={[styles.overviewCard, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.overviewNumber, { color: theme.colors.warning }]}>
          ...
          <View style={[styles.overviewCard, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.overviewNumber, { color: theme.colors.error }]}>
      ...
      <View style={styles.searchContainer}>
        <View style={[styles.searchInput, { backgroundColor: theme.colors.surface }]}>
          ...
          <TouchableOpacity style={styles.filterButton}>
            <Feather name="filter" size={20} color={theme.colors.textTertiary} />
          </TouchableOpacity>
        </View>
      </View>
